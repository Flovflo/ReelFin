...
- Liquid Glass follow-up: both tvOS playback routes now match the full native reference hierarchy with an 80-point inset, 360-point bottom gradient, episode/title row, three 70-point circular Subtitles/Audio/Video controls, full-width timeline with playhead-aligned elapsed time, and Info/InSight/Continue Watching glass pills. Info and Video expose playback quality/route; InSight exposes only honest Jellyfin item metadata; Continue Watching resumes paused playback and hides chrome.
- Final evidence: 91 focused player/layout tests passed, iOS 27 and tvOS 27 builds passed after `xcodegen generate`, and the authenticated Star City S1E1 XCUIRemote journey passed through hidden chrome, all track/video/info/insight panels, Continue Watching, focus restoration, and Menu precedence. Captures are exported under `/tmp/ReelFinLiquidGlassAttachments2`.
