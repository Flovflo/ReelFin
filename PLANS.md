...
## HEVC HLS Manifest Compatibility Gate - 2026-08-10

- Replace the hard-coded HEVC HLS codec with a checked ISO/IEC 14496-15 Annex E projection of the effective sample entry and source `hvcC` general profile/tier/level fields. Reject absent, malformed, or truncated decoder configuration before generating media.
- Advertise PQ/HLG only from explicit transfer or retained Dolby Vision/HDR range evidence. Main 10 SDR must omit `VIDEO-RANGE` and retain an honest SDR capability expectation.
- Normalize samples from an explicit validated `hvcC`/`avcC` NAL length format before considering Annex-B start codes. Keep every advertised media segment independently decodable by retaining its boundary keyframe, requiring startup audio within a bounded grace, and failing closed when the sample/byte/duration boundary-search budget is exhausted; Task 4 may tune those budgets without restoring mid-GOP output.
...
## Checked Matroska/EBML Bounds - 2026-08-10

- Make `EBMLReader` the single authority for checked payload ranges: exact size conversion, overflow-reporting endpoint addition, parent containment, available-data containment, and forward cursor progress.
- Route Track, Cluster/BlockGroup, Segment/Info, Cue, SeekHead, and streaming-demux loops through validated ranges before reads, slices, or cursor advancement. Propagate malformed SeekHead structure as `EBMLError` instead of silently discarding it.
- Allow an unknown-size root Segment only with an explicit available enclosing end; reject unknown-size nested elements. Preserve zero-size payloads and the incremental first-probe behavior for known-size Segments whose later clusters are not available yet.
...
## Build 17 Opaque Playback Evidence - 2026-08-11

- Correlate public media, source, track, server, and path identities with one process-local random 256-bit HMAC-SHA256 key and domain separation. Emit fixed 16-character lowercase hex values, use `unknown` for absent values, and keep playback session IDs independently random.
- Replace free-form deep playback evidence with typed JSONL for the closed events `plan`, `routeSelection`, `audioSelection`, `firstFrame`, `ttff`, `avPlayerTick`, `playbackProof`, and `sampleBufferTick`. Restrict values to booleans, finite numbers, closed categories, and opaque correlations.
...
- Reuse a one-shot raw-original source resolution across the custom-to-native handoff only when item and authenticated session match. A nil-tick focus warm may serve an exact Resume request because resume remains a client-side native demux seek; adaptive/session URLs are never retained.
- When a native focus/detail prewarm is already complete, present the native player directly rather than briefly presenting and replacing a disposable custom surface. Preserve the requested resume ticks in the native snapshot.
- Keep landscape requests idempotent across presentation handoff. Record monotonic startup stages with opaque session, app build, and platform rather than deriving latency from wall-clock time.
- Switch Matroska audio in place: latest request wins, the selected track commits only on its first decoded audio sample, and only the audio renderer/queue/timing state is flushed and reprised. Preserve the reader generation, video decoder/queue/display surface, synchronizer clock, and visible frame.
- If a requested audio decoder cannot be prepared, retain the previously audible track, clear the pending UI selection, and report the failure without restarting playback. Do not lower startup or rebuffer thresholds as part of this latency correction.
- Keep native handoff freshness source-coupled through a one-shot claim: 180-second monotonic lifetime, 64-entry hard capacity, deterministic eviction, and invalidation visible to the prewarmer. The native surface alone owns probe/demux; a trusted raw-original handoff must not pre-open the byte source in the controller.
- Count first frame for the sample-buffer route only after the active generation enqueues its first video sample, then commit the same TTFF metric, health, watchdog, trace, evidence, and total-signpost lifecycle as AVPlayer. Audio enqueue, playback-time callbacks, stale generations, and repeated video samples must not complete the startup stage.
- When audio selection coincides with seek, source replacement, or subtitle restart, preserve the pending track through that operation and commit/fail it on the new reader's first audio sample without a second reader or renderer restart.
- Treat Jellyfin stream metadata as display-only hints during the trusted handoff. The surface-owned Matroska open remains authoritative for EBML TrackNumber IDs and merges titles, languages, codecs, and flags without using Jellyfin IDs for native selection.
- Bound a pending restart/audio switch by selected-audio and total packet budgets. Every demuxed packet counts, including video, subtitle, and unselected audio; decoder throw, nil samples, or EOF restore the committed decoder and fail the pending UI choice without flushing video.
...
## Build 17 Artwork Resource Bounds - 2026-08-12

- `DefaultImagePipeline` now applies injectable defaults of 12 MiB encoded bytes, 40,000,000 source pixels, and four global loads. The global permit is admitted after per-URL deduplication and remains held through disk validation, transport, ImageIO validation/downsampling, memory insertion, and disk write. A 34-file local PNG/JPEG/WebP corpus measured 7,473,677 bytes maximum encoded size and 4,640×1,440 / 6,681,600 maximum pixels, so the defaults are 1.68× and 5.98× those checked-in maxima; this does not weaken server-side rejection for larger artwork.
- Artwork transport streams bytes, rejects non-2xx status/non-image MIME/oversized declared length before reading a body, and rejects cumulative overflow while streaming. Cached bytes receive the same encoded and ImageIO-dimension checks before decode; malformed or out-of-policy cache entries are evicted.
- ImageIO source dimensions are checked with overflow-safe multiplication before thumbnail decode. The previous `UIImage(data:)` fallback is removed, so unrecognized payloads cannot reach an unbounded decode path.
- `JellyfinAPIClient.prefetchImages` now selects URLs only and hands them to the composition-root-injected Shared image pipeline. This preserves authenticated headers, sanitised cache keys, bounded transport, decoding, and deduplication without an `ImageCache` dependency cycle.
- Focused evidence: all 27 `ImageCacheTests` passed on iPhone 17 / iOS 26.5 in 1.541 seconds, including declared/cumulative/invalid/negative/lying-short length limits, MIME/status, network and disk pixel rejection with zero decoder calls, boundary success, global N=2 admission, deterministic cancellation at the exact grant handoff, cancellation during suspended network or cached decode with no unintended cache side effects, token timing, cache sanitization, and same-URL single-permit/single-decode deduplication. Cancellation during a valid cached decode preserves the disk entry, allows a fresh retry, and recovers the permit; malformed and out-of-policy entries still evict. Two `JellyfinImageURLTests` passed in 0.006 seconds and prove deduplicated primary/backdrop forwarding, including an episode's parent backdrop. The token lookup occurs only after admission and a disk miss, immediately before transport; queued and valid disk hits perform no lookup. No live server operation was performed.
- Rereview follow-up: permits are now lease-owned in the limiter actor, so cancellation after a waiter is granted returns that exact permit exactly once. `cachedImage` also acquires the same global limit before reading and decoding, and `LRUDiskCache` compares the on-disk file size against the caller limit before allocating `Data`; oversized/tampered entries are evicted. Cancellation is rechecked after decoding and immediately before memory/disk side effects.

## Build 17 Cache Loader Concurrency - 2026-08-12

- Both progressive playback lanes now use scoped `NSLock.withLock` state updates under Swift 5.9; neither `CacheResourceLoaderDelegate` nor `LocalCacheHTTPServer` calls `lock()` or `unlock()` from an asynchronous function.
- Playhead publications carry a monotonic revision and pass through one actor, so a suspended older update cannot overwrite the latest seek/starvation target. The resource-loader task map installs a token before starting work and only attaches or completes that exact token, closing cancel-before-attach and stale-completion removal races.
- Focused evidence: 8/8 publication/state/registration tests and 4/4 cache lifecycle tests passed on iPhone 17 / iOS 26.5. Three credential-dependent cache-loader integration tests skipped explicitly. Fresh iOS and tvOS 26.5 builds succeeded, with zero async lock/unlock diagnostics in either scoped source file. Swift language mode remains 5.9.

## Build 17 No-Publication Release Gate - 2026-08-12

- The final code gate is PASS on `73eef4976d909c4a6453587879e314b9e100401c`. The only delta after the complete `cd2cb32` matrix is the iPad UI-test correction: it finds the native sidebar cells, taps Home/Search/Settings, and proves the expected destination instead of requiring a descendant accessibility element to be directly hittable. A fresh iPad Pro 13-inch (M5), iOS 26.5 run passed the two adaptive tests plus the App Store capture test, 3/3 with zero failures; an independent rereview repeated the 3/3 gate, found no issue, and passed whitespace validation.
- The no-publication matrix passed 55/55 isolated-uv ScriptTests, release preflight, the deterministic AVFoundation gateway test, 160 focused-A non-live tests with six explicit live skips, 129/129 focused-B playback/HLS/native tests, 8/8 playhead-publication tests, the full iPhone unit gate with 1,269 executed tests and ten explicit live/fixture skips, and ReelFinTV 18/18 plus a separate successful tvOS build. No archive, upload, distribution, submission, Apple-state mutation, or push belongs to this gate.
- All nine security findings remain represented in the release evidence: Quick Connect origin validation; checked HTTP Range arithmetic; loopback capability/admission/lifecycle controls; bounded independent synthetic HLS; checked Matroska/EBML ranges; fail-closed URL diagnostics; opaque bounded playback evidence; uv-only secret-safe QA; and bounded artwork transport/decode/concurrency. Exact Matroska local-endpoint, Python-boundary, shell-syntax, raw-lock, whitespace, fatal-runtime, and scoped lock-warning scans were clean.
- Measured bounds remain explicit rather than universal claims: local playback capacity is 24 from a measured progressive AVPlayer peak of 12; HLS boundary search is capped at 4,096 reads, 64 MiB, or 12 seconds; the HLS working set defaults to 12 retained segments; artwork defaults are 12 MiB encoded, 40 MP, and four concurrent loads; deep evidence is capped at 1 MiB; and native handoff state has a 180-second lifetime and 64-entry capacity.
- Residual code debt is recorded, not hidden: cold compilation still reports Swift 6-forward actor-isolation warnings in `SampleBufferDisplayLayerRenderer` and the audio-interruption path in `CustomPlaybackEngine`, plus deprecated AVFoundation sample-buffer APIs. The project currently builds with Swift 5.9, so these are not current build failures, but they must be removed before enabling Swift 6 language mode.
- External release readiness remains separate from the code PASS. The seven live Jellyfin E2E inputs and the three App Review demo credentials are absent; the historical review password still requires rotation; Xcode 26.6 first-launch setup exits 69; DSA is a human/legal decision; physical-device HDR/Dolby Vision and arbitrary remote-server/media behavior remain unproved. App Store privacy is already published as Data Not Collected, `asc doctor` reports a valid ReelFin Admin profile, and the deterministic storefront reviewer corpus passed 6/6 UI invocations, 12/12 strict bit-identical pairs, manifest verification, and visual inspection.

## Custom Player Track Controls And Dismissal - 2026-08-15

- The custom iOS player now exposes an explicit close button and a `Pistes` sheet for the engine's audio and external subtitle tracks; the user no longer has to wait for AVKit's auto-hide timer to leave playback.
- Subtitle selection reports a visible loading state, applies the first cue at the current playback position immediately after parsing, and keeps video running when a sidecar fails. The error remains visible as a retryable track-status message instead of collapsing the whole player into failure.
- Audio selection retains a pending request while AVFoundation finishes its media-selection setup, applies it only after verifying the selected option, and reports an explicit application error when the option cannot be selected.
- RED/GREEN coverage includes pending subtitle feedback/immediate cue application, non-fatal sidecar failure, and queued audio-selection policy. iOS custom-player tests passed on iPhone 17/iOS 26.5; XcodeGen and iOS/tvOS 26.5 builds passed.
