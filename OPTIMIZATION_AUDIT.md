...
## Native VLC-Class Player Performance Note - 2026-04-23
...
- The first tvOS Matroska stutter log showed repeated `nativevlc.audio.rebuffer` with `audioAhead=0` despite hundreds of audio packets. The fix now synthesizes missing Matroska audio packet durations, advances laced E-AC3 PTS in streaming clusters, normalizes duplicate compressed-audio PTS values at the renderer boundary, and requires multiple starvation ticks before pausing the render synchronizer.
- The follow-up stutter log showed `nativevlc.buffering.ready` with `audioPackets=1` and `audioAhead=9.632`, proving the policy was counting local Swift queue duration as if it were already primed inside Apple’s audio renderer. Startup now requires 32 renderer-accepted audio samples, rebuffer resume requires 16, diagnostics expose `primed video/audio`, and the audio renderer is fed by `requestMediaDataWhenReady` instead of a fixed polling timer.
- The next runtime log showed `nativevlc.buffering.ready` with `audioPackets=32`, `audioPrimed=32`, and `audioAhead=10.624`, moving the remaining risk to the audio timeline/renderer boundary. The Matroska audio timing normalizer now collapses forward gaps as well as duplicate/backward timestamps and reports `maxAudioPTSCorrection` in diagnostics. Audio rebuffering also moved from callback-count based gating to sustained starvation time, because `requestMediaDataWhenReady` can fire repeated ready callbacks in milliseconds and must not pause the render synchronizer for transient local queue gaps.
...
- Remaining performance risk: compressed E-AC3 renderer behavior still needs validation on real Apple TV hardware and a signed-in Jellyfin runtime session; simulator HAL audio errors are noisy and not enough to prove hardware continuity. If cuts persist while `audioRebuffers` stays flat and Dolby multi-frame timing is active, or if `nativevlc.audio.startup_degraded` appears for playable Dolby tracks, the next implementation step is a decoded PCM `AudioConverter`/`AVAudioEngine` path for E-AC3 rather than compressed sample-buffer audio. The current seek control restarts the native reader at the requested time; an in-place demuxer seek API remains the cleaner long-term control path.
