...
- Superseded the iOS metadata-only high-risk Direct Play preemption. The latest `2050da6b` log showed `warmup.selection.hit` followed immediately by `ios_high_risk_progressive_directplay`, so ReelFin was forcing HLS from source shape before measuring a failure. iOS now keeps original Direct Play until measured startup/session health proves it unsafe; post-start stalls and AVPlayer failures remain handled by the existing recovery policy.
- Latest validation after disabling metadata-only Direct Play preemption: targeted red test failed before implementation and now passes; focused Direct Play route-guard suite passed; `xcodegen generate`, iOS `ReelFin` build, tvOS `ReelFinTV` build, and live player E2E passed with artifacts `.artifacts/player-e2e/20260505-185957` (`4/4` explicit probes, resume reporting, `4/4` original-stream benchmarks, `3/3` live playback probes, `74/74` deterministic tests, clean fatal-log scan).
