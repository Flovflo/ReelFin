...
## Unified 0.1.2 Build 16 Playback Reliability Gate - 2026-07-17

- Treat the requested start position as part of a prewarmed playback identity. A Resume-scoped warm route must never be consumed by Restart, and a Restart route must resolve explicitly from the beginning.
- Keep one focus owner for the invisible tvOS Remote input layer. Its callers own focus through `FocusState`; the layer must not also declare itself the default focus target.
- Preserve the authenticated Apple TV simulator and validate Star City S1E1 with alternating Continue/Restart, auto-hide without a replayed direction, real Skip Intro, advancing video, and accepted audio before archive.
...
- Review follow-up: make each registered-image consumer wait cancellation-responsive through a one-shot continuation gate, reject pre-cancelled registration, use cancellation-aware task-group admission, and reject a pre-cancelled image load before it can mutate generation ownership.

## Cinematic Editorial Home Composition - 2026-07-26

- Recompose the iOS Home hero as kicker, cache-first logo or immediate title fallback, one metadata line, and a stable Play/Favorite/More control cluster. Keep the backdrop itself tappable, preserve featured transition IDs and playback routing, and give More its own native button and UI-test identifier.
- Own iOS hero rotation inside `HeroCarouselView`: advance only for an active, unassisted, idle scene; calculate pages through `HeroRotationPolicy`; use a local 0.20–0.22-second animation; and reserve haptics for direct paging. Reduce Motion, VoiceOver, touch/drag interaction, and background scenes suppress automatic rotation.
- Reuse the same canonical `.logo` identity on iOS hero, tvOS hero, and immersive landscape rails. Resolve URLs through the authenticated API client, consult the shared cache before transport, crop cached and downloaded logos with `TransparentImageCropper`, and guard every suspension plus final publication against cancellation and stale item identity.
- Apply native Liquid Glass only to adjacent action controls, individual section chevrons, and the focused tvOS shelf card. Resting media stays artwork-only; Reduce Transparency selects opaque tonal controls; promo and quality badges remain static tonal surfaces; tvOS native buttons, focus IDs, move commands, and action order remain unchanged.
- Reduce `CinematicBackdropView` to one resolved source and exactly two canonical requests: one blurred `.heroLow` and one sharp `.heroHigh`. Remove the fallback/current overlap, second blur, material wash, and full-screen drawing group while retaining cheap gradient scrims and sharp-layer visibility telemetry.
- Required validation passed on Xcode-beta and the configured 26.5 simulators: 24/24 Home policy/layout tests, 4/4 Home/detail-action UI tests, `xcodegen generate`, the iPhone 17 `ReelFin` build, and the Apple TV 4K (3rd generation) `ReelFinTV` build.

## Library Intent Ownership Hardening - 2026-07-26

- Replace independent Library load/search/pagination work with model-owned criteria and pagination tasks. Every criteria submission captures one immutable, trimmed `LibraryCriteria`, advances its generation, cancels and invalidates both prior task classes, resets only page/end markers, and leaves committed items visible for cached-first repainting.
- Give pagination its own request ID and loading-owner token. A page request captures generation, criteria, request ID, and page before suspension; a newer filter, sort, search, cancellation, or page request prevents stale results, page markers, errors, and deferred loading cleanup from touching current state.
- Check both cancellation and matching ownership after every repository/API suspension, including cached/remote view resolution, view persistence, local item/search reads, remote item fetches, and item upserts. Query construction, view scoping, merging, and sorting use captured criteria only.
- Preserve local-search plus remote-merge behavior, cached browse plus remote replacement, 120/48 page sizes, page numbering, view-ID scoping, search pagination suppression, canonical deduplication, and playback-quality preference. No API authentication, artwork warmup, playback warmup, focus topology, or `MainActor` scope changed.
- Own the 250 ms search debounce separately from artwork/playback warmup. Filter and sort changes cancel it and submit immediately; pagination is submitted synchronously through the model; disappearance cancels debounce and model intents; returning resubmits current criteria without clearing committed content.
- Deterministic RED failed with exit 65 on the missing model-owned submission/cancellation APIs. GREEN passed all 6 `LibraryViewModelTests` with zero failures. XcodeGen generation and sequential iPhone 17/iOS 26.5 plus Apple TV 4K (3rd generation)/tvOS 26.5 builds succeeded, and `git diff --check` passed.

## Cinematic Editorial Library Composition - 2026-07-26

- Recompose iOS Library around one `StickyBlurHeader` scroll owner. Its stable content is an editorial kicker/title, truthful loaded-result context, the only bound search field, Movies/Shows/Sort controls, and the existing lazy poster grid; the pinned compact title/filter/sort row remains mounted at fixed geometry and becomes hittable only after meaningful scroll progress.
- Quantize reveal work to 24 buckets before state publication and enter compact presentation only at bucket 18 (120 points of a 160-point reveal). `.always` headers install no geometry tracking, while reveal headers ignore repeat bucket values. Preserve grid identity, transition namespaces, poster metrics, and the complete Task 5 debounce/criteria/pagination lifecycle.
- Put each iOS and tvOS filter/sort control in one stable adjacent-control `GlassEffectContainer`, with the material owned by each native interactive control. Keep title, context, search artwork, control containers, and resting cards free of glass; Reduce Transparency bypasses live blur sampling and uses the opaque editorial surface with a stronger stroke.
- Keep tvOS Library buttons, IDs, selection, first-row Up route, Detail transition provenance, and exact poster restoration unchanged. Focused posters alone receive the `focusedMedia` surface; resting posters use a cheap fill, fixed geometry, and no live glass or rim. Selection is synchronous through `TVLibraryActivationPolicy`, with press opacity always and press scale only when Reduce Motion is off; no fixed activation sleep remains.
- Deterministic RED failed only on the requested missing presentation, tracking, and activation seams. GREEN passed the focused 38-test Library/layout/navigation/transition suite, including mutation proof for the no-sleep source contract. The mock iPhone journey passed expanded → compact → restored in an asserted portrait viewport with three retained screenshots, and passed again at Accessibility Large Text. The app's current orientation lock prevented a genuine Library landscape journey in this harness, so landscape remains explicitly not run. XcodeGen generation and sequential iOS/tvOS 26.5 simulator builds are required as the final gate; authenticated live Jellyfin and interactive tvOS journeys remain separate evidence and must not be inferred from mock data.
- Review follow-up centralizes the actual tvOS Library grid and first-row focus resolver on the same 56-point side inset, 240...280-point adaptive width, and 32-point horizontal spacing, so Full HD renders six cards and all six receive Up routing. Header context now describes only committed visible count plus updating state; pending search/filter controls can no longer relabel the intentionally preserved prior grid.
- Simulator screenshot review moved Liquid Glass from detached background shapes onto each control's laid-out label surface, restoring high-contrast Movies/Shows/Sort text while preserving one effect per control. Compact Accessibility Large keeps fixed geometry with bounded label scaling and an icon-only Sort surface carrying an explicit current-sort accessibility label; both normal and Accessibility Large mock journeys pass with expanded, compact, and restored captures.

## Cinematic Editorial Detail Composition - 2026-07-26

- Reduce the selected iOS Detail page to one hero-grade composition: the current item renders the surviving `HeroBackgroundView` once with canonical `.heroLow` and `.heroHigh` requests, while every neighbor renders one canonical `.landscapeRail` image plus a static legibility gradient and its lightweight preview copy. The explicit budget is one hero stack/two image layers for the selected item and zero hero stacks/one image layer per neighbor.
- Project raw iOS scroll geometry into an Equatable presentation before state publication: 32 hero steps, 12 chrome steps, and a separately derived raw horizontal-selection lock. Under Reduce Motion the visual state is expanded or collapsed only. Keep supporting-row spacing fixed and keep selected/preview shadow geometry fixed at radius/y `30/24` and `18/12`.
- Quantize tvOS hero collapse to 16 visual steps while deriving preview interaction from the raw threshold. Keep its rendered shadow fixed at radius/y `40/24`; remove continuous shadow geometry and the fixed 320 ms focus-completion sleep. Scroll handoff now finishes from `.logicallyComplete` animation completion and retains request-ID stale-completion protection.
- Use `EditorialMediaIdentityView` for iOS and tvOS Detail identity with an immediate text fallback, media kicker, and one concise metadata line. Canonicalize cast artwork through `.avatar`; static cast remains readable but is no longer a focus destination, and More Like This Up routing ignores it.
- Keep iOS Back as an independent circular control. Coordinate Share/More and Play/Favorite/Watched in stable `GlassEffectContainer`s while applying interactive material—or the opaque Reduce Transparency fallback—to each native control itself. More remains a visible menu with the existing Download action, alert, and stable identifiers. On tvOS, keep the existing Play/Watchlist/Watched order, bindings, default Play focus, IDs, size, and inline-player behavior while coordinating the three native controls in one stable container.
- Deterministic RED failed on the absent artwork/presentation/focus seams, then the source-wiring RED isolated the fixed 320 ms sleep. GREEN passed the complete 117-test Detail/navigation/playback regression selection. The updated six-test mock UI class covers Home and Library Detail entry/return, More/Download, state toggles, and adjacent carousel selection with retained screenshots; the More/Download journey also passed after rebooting the iPhone 17 simulator with actual Reduce Motion and Reduce Transparency preferences enabled. XcodeGen generation and sequential iOS/tvOS 26.5 simulator builds passed. No authenticated live Jellyfin or interactive tvOS Siri Remote journey was run, so neither is claimed.
- Retained Detail screenshots exposed the same detached-background Glass compositing failure previously found in Library: labels and glyphs could be washed out even though their accessibility elements remained hittable. iOS and tvOS Detail controls now attach Glass to each complete laid-out label surface; a source regression rejects detached material backgrounds, and the recaptured iOS action hierarchy shows readable Play, Back, Share, More, Watched, and Favorite controls.
- Independent review moved iOS scroll presentation into Observation-backed top-stage and compact-header children so bucket changes no longer re-evaluate the screen that invokes lazy supporting content. It also wired the artwork composition and tvOS action-order policies into rendered views, restored the specified iOS Play/Favorite/Watched order, made compact chrome destroy live blur under Reduce Transparency, exposed kicker/title/metadata to VoiceOver without repeating the media type, and hid neighbor artwork from accessibility traversal.
- The post-review Detail/navigation/playback selection passed 126/126, the complete mock Detail UI class passed 6/6, and fresh XcodeGen, iOS 26.5, and tvOS 26.5 gates passed after the corrections.

## Cinematic Editorial Final Delivery - 2026-07-27

- Close the visual system as one restrained Cinematic Editorial Glass+ language across Home, Library/Search, and Detail. Native Glass belongs to complete interactive control surfaces and focused tvOS media only; title, synopsis, backgrounds, section bodies, resting cards, and containers do not receive redundant material. Reduce Transparency destroys live blur/material branches in favor of opaque editorial surfaces, and Reduce Motion removes carousel automation or transform-heavy transitions without changing routes.
- Keep all hot interaction paths intact: cache-first authenticated Home paint, model-owned latest-wins Library criteria/pagination, synchronous tvOS card activation, occurrence-qualified Detail transitions, exact Home/Library focus return, native playback planning, and cancelable session-scoped warmup. The final independent review found no remaining P0-P2 issue.
- Finish artwork scheduling with a shared production pipeline speculative admission cap of four, a 24-URL candidate ceiling, six image transport connections per host, and two ImageIO decode operations. Shared work tracks every consumer's priority and demotes after a visible consumer releases; visible callers bypass speculative admission. Sync warms both low and high featured hero variants, episodes use series backdrops for landscape/hero roles, and shimmer animation is limited to the low hero placeholder.
- Fresh result bundles on explicit installed runtimes are green: iPhone 17/iOS 26.5 passed 1,148/1,159 with zero failures, 11 explicit fixture/environment skips, and zero runtime warnings; Apple TV 4K (3rd generation)/tvOS 26.5 passed 51/51 with zero failures, skips, or runtime warnings after restoring its private authenticated session. tvOS includes 18 unit tests, 23 auth/focus UI tests, and ten live journeys with exact focus return, repeated Continue/Restart, rendered video/audio, and real Skip Intro.
- Retained simulator evidence covers iOS normal Home, Detail, expanded/compact Library, Reduce Motion + Reduce Transparency, and Accessibility Large; tvOS evidence covers authenticated Home, focused landscape/poster rows, Library controls/grid, Detail, player menus, and Skip Intro. The live fixture advertised primary art for only 32/258 cached items and backdrops for 5/258; the pipeline still downloaded, decoded, cached, and displayed valid WebP responses, while missing server art remained a nonanimated placeholder.
- The standard player E2E driver was exercised rather than used as a substitute for the result-bundled schemes. Original-file probes and benchmarks passed 4/4 each; its optional HLS transcode health probe consistently received server HTTP 500 for 8/8 segments. With that external diagnostic explicitly budgeted, deterministic playback and deep MP4 passed. The sequential simulator HDR render marker failed after MP4 but passed an isolated 120-second rerun; isolated MKV/SampleBuffer did not publish its render marker. Resume state restoration passed for both isolated targets. These limitations are retained privately and are not represented as green.
- Instruments SwiftUI/Animation Hitches capture was unsupported on the installed simulator runtime, while Time Profiler failed to finalize under Xcode beta. No before/after timing claim is made. The performance acceptance evidence is the warning-free complete schemes, focused 132-test editorial/image selection, 11/11 retained UI selection, bounded concurrency tests, quantized Observation surfaces, and inspected screenshots/signposts.

## Real Jellyfin Artwork And Search Polish - 2026-07-29

- Treat image tags as a speculative-prefetch hint only. Visible artwork always resolves through the authenticated pipeline so incomplete Jellyfin metadata cannot suppress a valid image; actual 404s enter a five-minute session negative cache and final misses receive a deterministic static editorial fallback.
- Replace the Library/Search expanded-versus-compact switch with one continuous complementary crossfade. Keep one stable scroll/grid owner, make exactly one filter cluster interactive and accessible at a time, and preserve Reduce Transparency plus Reduce Motion behavior.
...
## Native Liquid Glass Player Redesign - 2026-08-23

- Keep one ReelFin-owned chrome above each Apple-native playback surface. AVKit's duplicate playback controls stay disabled on the custom `AVPlayer` route; background tap hides or reveals the ReelFin chrome immediately.
- Match the supplied iOS reference with real Liquid Glass controls: Close and AirPlay at the top, the system volume surface opposite them, 10-second transport around a primary play/pause control, episode identity plus track actions above one full-width timeline. Unsupported PiP/share actions remain absent instead of becoming decorative buttons.
- Make audio and subtitle changes explicit state transitions. A selected row becomes pending with “Changement…”, stays open until the actual sample-buffer renderer or AVPlayer media-selection group confirms the selected track, and exposes a bounded failure instead of silently closing while the old language continues.
- Match the supplied tvOS reference with four focusable circular actions (`Vidéo`, `Sous-titres`, `Audio`, `Réglages`), media identity and one timeline. Each action owns a real destination; the settings panel exposes Info, Détails, and Continuer. Down opens that panel in one press from either the visible timeline or hidden chrome and deterministically focuses Info.
- Present one compact vertical Continue/Restart choice before player/session creation whenever saved progress is meaningful on both iOS and tvOS. Cancel never launches playback.
- Preserve latest-wins/cancelable player tasks and Apple playback frameworks. The redesign adds no third-party media engine, no private API, and no broader `MainActor` network or decode work.
- Validation on Xcode beta and the installed 26.5 runtimes: XcodeGen passed; the complete `PlaybackEngineTests` bundle passed 1,117 tests with 9 external-fixture skips and zero failures; `ImageCacheTests` passed 27/27; `ReelFinTVTests` passed 18/18; the deterministic iOS interaction test exercised hide/reveal, play/pause, seeking, audio, subtitles, dismissal/reopen, and video information; the hermetic tvOS UI test proved hidden-chrome Down → Settings → focused Info → opened Info in one route; final explicit iOS and tvOS builds both succeeded. Inspected captures remain under `.artifacts/player-reference-validation-ios-final/` and `.artifacts/player-reference-validation-tvos-final/`.
- The repository live-player probe was also attempted against the simulator's preserved account. Both cases stopped before playback because the configured explicit Jellyfin movie/series IDs were not visible in that account (`explicitTargetNotVisible`); this is retained as an external fixture mismatch, not represented as live playback evidence.
